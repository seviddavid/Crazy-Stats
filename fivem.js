...
global.icon = `https://i.imgur.com/OhiQkCa.png`;
...
client.on("message", message => {
  ...
  if (set.has(message.author.id)) {
    message.delete();
    return zembed(`<@${message.author.id}>, Te rog asteapta 3 secunde inainte sa folosesti comanda.`)
  }
...
return Math.floor(Math.random() * (max - min)) + min;
 ...
 /*  module.exports.log = function (content) {
  ...
  try {
    ...
    zembed("Scuze, aceasta comanda nu exista.\n\n**Console**: ```js\n" + err + "```")
  }
});
...
client.on('guildMemberAdd', member => {
  ...
  .setColor(0x16ff00)
  .setTitle(`BUN VENIT`)
  .setField(`Salut ***${member.nickname}***, Bine ai venit pe comunitatea Crazy Romania.\nSper sa iti placa in orasul nostru, si te invit sa citesti regulamentele server-ului !`)
  .setImage(member.user.avatarURL)
  .setFooter(`seviddavid`, icon)

  member.guild.channels.get(`690664508145926144`).send({ embed: embed});
});
